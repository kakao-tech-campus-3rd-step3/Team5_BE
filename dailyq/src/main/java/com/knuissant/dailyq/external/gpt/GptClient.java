...
public class GptClient {
    ...
    public FollowUpQuestionResponse callForFollowUp(String systemPrompt, String userPrompt) {
        ...
        try {
            return chatClient.prompt()
                    ...
                    .user(userPrompt)
                    .call()
                    .entity(FollowUpQuestionResponse.class);
        } catch (NonTransientAiException e) {
            log.error("[GPT] Non-transient error for follow-up questions: {}", e.getMessage());
            throw new NonRetryableInfraException(ErrorCode.GPT_API_COMMUNICATION_ERROR);
        } catch (TransientAiException e) {
            log.error("[GPT] Transient error for follow-up questions: {}", e.getMessage());
            throw new NonRetryableInfraException(ErrorCode.GPT_API_COMMUNICATION_ERROR);
        } catch (ResourceAccessException e) {
            log.error("[GPT] Network error for follow-up questions: {}", e.getMessage());
            throw new RetryableInfraException(ErrorCode.GPT_API_COMMUNICATION_ERROR);
        } catch (Exception e) {
            log.error("[GPT] Unexpected error for follow-up questions: {}", e.getMessage());
            throw new NonRetryableInfraException(ErrorCode.GPT_API_COMMUNICATION_ERROR);
        }
    }
